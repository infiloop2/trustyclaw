...
AGENT_CHAT_THREADS: dict[str, dict[str, Any]] = {
    "website-redesign": {
        ...
        "updated_at": "2026-06-08T00:00:30Z",
    }
}
...
def route_app_api(
    ...
    if method == "GET" and relative == "health":
        return {"status": "ok", "app": "agent_chat", "mock_backend": True}
    if method == "GET" and relative == "preferences":
        return {"preferences": {"density": "comfortable", "show_completed": True}}
    if method == "PUT" and relative == "preferences":
        if not isinstance(body, dict) or not isinstance(body.get("preferences"), dict):
            raise api_error(HTTPStatus.BAD_REQUEST, "preferences must be an object")
    ...
    if method == "POST" and match:
        ...
        AGENT_CHAT_THREADS[thread_id]["updated_at"] = "2026-06-08T00:09:00Z"
...
def _list_threads(host_api: HostApi) -> list[dict[str, Any]]:
    ...
    for app_thread in AGENT_CHAT_THREADS.values():
        ...
        timestamps = [task.get("updated_at") or task.get("created_at") for task in tasks]
        threads.append(
            {
                "thread_id": app_thread["thread_id"],
                "agent_runtime": app_thread["agent_runtime"],
                "last_used_at": max(timestamps, default=app_thread["updated_at"]),
