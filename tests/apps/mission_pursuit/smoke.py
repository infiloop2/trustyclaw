...
WORKSPACE: dict[str, Any] = {
    ...
    "created_at": _BASE.format(0),
}
MESSAGES: list[dict[str, Any]] = [
    {"id": 1, "role": "user", "content": "Set up a launch tracker and check on it every morning.", "meta": None, "created_at": _BASE.format(1)},
    {"id": 2, "role": "agent", "content": "Done. I built the **Launch Tracker** artifact and scheduled a morning check.", "meta": None, "created_at": _BASE.format(2)},
    {"id": 3, "role": "event", "content": 'Created artifact "Launch Tracker"', "meta": {"action": "create_artifact", "artifact_id": "launch_tracker"}, "created_at": _BASE.format(2)},
    {"id": 4, "role": "event", "content": 'Scheduled "Morning check" every day', "meta": {"action": "create_schedule", "schedule_id": "morning_check"}, "created_at": _BASE.format(2)},
    {"id": 5, "role": "error", "content": 'Action rejected: artifact "old_notes" does not exist', "meta": {"action": "update_artifact"}, "created_at": _BASE.format(3)},
]
SCHEDULES: list[dict[str, Any]] = [
    {
        ...
        "enabled": True,
        "created_at": _BASE.format(2),
        "updated_at": _BASE.format(2),
        "last_run_status": "completed",
        "last_run_at": _BASE.format(4),
    }
]
...
MEMORIES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "content": "Alex gives final approval for launch copy.",
        "updated_at": _BASE.format(4),
    },
]
...
def _append_message(role: str, content: str, meta: dict[str, Any] | None = None) -> None:
    MESSAGES.append(
        {"id": _NEXT_MESSAGE_ID[0], "role": role, "content": content, "meta": meta, "created_at": "2026-06-08T00:09:00Z"}
    )
...
def route_app_api(
    ...
    if method == "POST" and relative == "interactions":
        ...
        if control["type"] in {"toggle", "field"}:
            ...
            artifact["updated_at"] = f"2026-06-08T00:{_NEXT_UPDATE_MINUTE[0]:02}:00Z"
    ...
    if match:
        ...
        if method == "POST":
            ...
            memory["updated_at"] = "2026-06-08T00:09:00Z"
...
def desktop_smoke(page: Any) -> None:
    ...
    expect(frame.locator("#agent-settings-toggle")).to_contain_text("Claude Code · Opus · High")
    ...
    frame.locator("#agent-runtime").select_option("codex")
    ...
    frame.locator("#agent-model").select_option("gpt-5.6-luna")
    frame.locator("#agent-effort").select_option("max")
    frame.get_by_role("button", name="Apply changes", exact=True).click()
    expect(frame.locator("#agent-settings-toggle")).to_contain_text("Codex · gpt-5.6-luna · Max")
    expect(frame.locator("#feed")).to_contain_text("Switched to Codex")
    ...
    expect(frame.locator("#feed .msg.agent strong")).to_have_text("Launch Tracker")
    expect(frame.locator("#feed .event-chip").first).to_contain_text('Created artifact "Launch Tracker"')
    expect(frame.locator("#feed .msg.error")).to_contain_text("Action rejected")
    ...
    # Schedule row with immediate controls: pause, then resume.
    ...
    expect(frame.locator("#schedules")).to_contain_text("paused")
    frame.get_by_role("button", name="Resume").click()
    expect(frame.locator("#schedules")).not_to_contain_text("paused")
