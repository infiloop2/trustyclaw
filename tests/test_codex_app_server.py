...
def fake_account_server(result: dict[str, object]) -> str:
...
class CodexAppServerTests(unittest.TestCase):
    def account_status_with_result(self, result: dict[str, object]) -> tuple[str, str | None, str | None]:
        previous = codex_app_server_module.DEFAULT_COMMAND
        codex_app_server_module.DEFAULT_COMMAND = [sys.executable, "-u", "-c", fake_account_server(result)]
        try:
    ...
    def test_account_status_reads_account_id_from_helper_for_active_chatgpt_account(self) -> None:
        ...
        # account id. The id is read narrowly from Codex auth storage.
        ...
        with patch("host.runtime.codex_app_server.read_codex_account_id", return_value="acct_123"):
            self.assertEqual(self.account_status_with_result(result), ("active", None, "acct_123"))
