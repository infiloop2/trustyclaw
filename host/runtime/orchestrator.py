...
def _refresh_runtime_status_serialized(runtime_type: str) -> str:
    ...
    # persisted any completed-login notification. Capture the first trusted anchor
...
def _capture_completed_codex_login(runtime_type: str) -> None:
    ...
    device-code app-server and has therefore persisted the successful
    ...
    that completion for the exact login id; the account id comes from the
    app-server payload, never the agent-writable auth file.
    """
    if runtime_type != "codex":
        return
