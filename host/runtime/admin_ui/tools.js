...
import { $, badge, esc, formatUnixTime, notice, setHtml } from "./helpers.js";
...
function toolsMessage(message, isError) {
  const node = $("tools-message");
      ...
      <div class="integration-summary">
        ...
        <div class="integration-title">
          <div class="preset-with-info">
            ...
            <button class="info-button" data-action="toggle-tool-info" data-tool="${esc(tool.tool_id)}" data-info="tool:${esc(tool.tool_id)}" aria-label="${esc(tool.display_name)} details" aria-haspopup="dialog" aria-expanded="false">i</button>
          </div>
...
export function toggleToolExpansion(toolId) {
  ...
  if (expanded) loadToolApprovals(toolId).catch(error => toolsMessage(error.message, true));
}
...
function renderToolInfo(tool) {
  const setupRows = renderSetupGuide(tool.setup_guide);
  return `
    ${setupRows}
    <h3>${esc(tool.display_name)} actions</h3>
    <table>
      ${tool.actions.map(action => `
        <tr>
          <td><strong>${esc(action.id)}</strong></td>
          <td>
            <div>${esc(action.description)}</div>
            <div class="muted">${esc(action.data_policy || "")}</div>
          </td>
        </tr>`).join("")}
    </table>`;
}

// The manifest's setup guide: a single operator-facing string describing what
// to do on the third-party side (create the OAuth client, register the redirect
// URI, get an API key) before Configure/Connect will work.
function renderSetupGuide(guide) {
  if (!guide) return "";
  return `
    <h3>Setup</h3>
    <p class="setup-guide">${esc(guide)}</p>`;
}
...
export async function setToolEnabled(toolId, enabled) {
  try {
    ...
    await refreshTools();
  } catch (error) { toolsMessage(error.message, true); }
}
...
export async function saveToolConfig(toolId, key) {
  ...
  try {
    toolsMessage("");
    await api("PUT", `/v1/tools/${encodeURIComponent(toolId)}/config`, { key, value: input.value.trim() });
    input.value = "";
    await refreshTools();
  } catch (error) { toolsMessage(error.message, true); }
}
...
export async function completeToolConnect() {
  ...
  if (!params.get("code")) {
    toolsMessage(`Connect cancelled: ${params.get("error") || "no authorization code returned"}.`, true);
    return;
  }
  try {
    ...
    notice(`Connected ${toolId}${label ? ` as ${label}` : ""}.`);
  } catch (error) { toolsMessage(error.message, true); }
  try { await refreshTools(); } catch (_error) { /* keep the notice visible */ }
}
...
export async function decideToolApproval(toolId, approvalId, decision) {
  ...
  try {
    ...
    if (result && result.status === "failed") notice(`Approved action failed: ${result.error}`, "error");
    else notice(decision === "approve" ? "Approved and executed." : "Denied.");
  } catch (error) { notice(error.message, "error"); }
  try { await loadToolApprovals(toolId); } catch (_error) { /* keep the notice visible */ }
}
