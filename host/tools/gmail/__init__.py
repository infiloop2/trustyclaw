...
# Per-action data policies (shown to the operator next to each action).
GMAIL_READ_POLICY = (
    "Read-only. Sends the query string or ids to Google and returns account data "
    "(labels, message ids, headers, snippets, bodies, threads, drafts). Runs "
    "directly with no approval."
)
GMAIL_SEND_POLICY = (
    "Composes an email and sends it through the connected account. Queued for "
    "explicit approval; the message reaches Google only after you approve."
)
GMAIL_MESSAGE_ACTION_POLICY = (
    "Applies a message operation (archive, labels, trash/untrash, read/unread, "
    "star) to the listed messages. Queued for approval before any change reaches "
    "Google."
)
GMAIL_LABEL_ACTION_POLICY = (
    "Creates, renames, recolors, or deletes a Gmail label. Queued for approval "
    "before any change reaches Google."
)
GMAIL_DRAFT_ACTION_POLICY = (
    "Creates, updates, sends, or deletes a Gmail draft. Queued for approval "
    "before any change reaches Google."
)
...
MANIFEST = ToolManifest(
    tool_id="gmail",
    display_name="Gmail",
    description="Read, search, and act on the connected Gmail account. Sensitive actions are approval-gated.",
    connection="oauth",
    actions=(
        ActionSpec(id="search_messages",
            description="Search messages with a Gmail query string.",
            data_policy=GMAIL_READ_POLICY,
        ...
        ActionSpec(id="list_labels", description="List the account's labels.", data_policy=GMAIL_READ_POLICY, input_schema=_schema({}), output_schema=GMAIL_OUTPUT_SCHEMA),
    ...
    setup_guide=(
        ...
        "control, and add the redirect URI shown by Connect (the loopback admin origin, "
        "e.g. http://localhost:7443/oauth/callback) to its Authorized redirect URIs. "
        "Enable the Gmail API for the project. On the OAuth consent screen, publish to "
        "Production (or add the account as a test user) — Gmail scopes are restricted, so "
        "test-mode refresh tokens expire after 7 days. Then set GOOGLE_OAUTH_CLIENT_ID and "
        "GOOGLE_OAUTH_CLIENT_SECRET here, enable the tool, and Connect with that account."
    ),
)
