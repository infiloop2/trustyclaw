...
const PR_WORKFLOW_STEPS = [
  { title: "Request", sub: "repository and outcome understood" },
  { title: "Implement", sub: "focused branch and complete change" },
  { title: "Verify", sub: "local tests and full diff review" },
  { title: "Push", sub: "commit and pull request on GitHub" },
  { title: "Review", sub: "checks and comments resolved" },
  { title: "Ready", sub: "quiet green round; human merges" },
];
...
function api(method, path, body) {
  ...
  parent.postMessage({ type: "trustyclaw-app-api", request_id: requestId, method, path: "/v1/apps/software_builder/api" + path, body }, "*");
  ...
  try {
    if (!Object.keys(sessionOptions).length) {
      ...
      if (!response.session_options || typeof response.session_options !== "object") {
        throw new Error("Software Builder returned invalid agent settings");
      }
...
function renderGoal() {
  ...
  $("goal-banner").innerHTML = goal
    ? `<span class="goal-kicker">Building</span> ${esc(goal)}` +
      (measurement ? `<span class="goal-measurement"><span class="goal-kicker">Done when</span> ${esc(measurement)}</span>` : "")
    : "";
}
...
function renderPrWorkflow() {
  $("pr-workflow").innerHTML = PR_WORKFLOW_STEPS.map((step, index) => {
    return `
    <div class="workflow-step">
      <span class="step-index">${index + 1}</span>
      <span class="step-main">
        <span class="step-title">${esc(step.title)}</span>
        <span class="step-sub">${esc(step.sub)}</span>
      </span>
    </div>`;
  }).join("");
}
...
function renderSchedules() {
  ...
  if (!rows.length) {
    $("schedules").innerHTML = `<div class="empty-state">No follow-ups scheduled. The agent can add a one-shot CI or review check.</div>`;
    return;
  }
...
function renderArtifacts() {
  ...
  if (!rows.length) {
    $("artifacts").innerHTML = `<div class="empty-state">No pull requests yet. Name a connected repository and the change you want.</div>`;
    return;
  }
...
function renderTools() {
  ...
  if (!rows.length) {
    $("tools").innerHTML = `<div class="empty-state">No optional research tools recorded.</div>`;
    return;
  }
...
function renderMemories() {
  ...
  if (!rows.length) {
    $("memories").innerHTML = `<div class="empty-state">Nothing remembered yet. The agent stores durable design decisions here.</div>`;
    return;
  }
