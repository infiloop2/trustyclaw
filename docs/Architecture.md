...
before launching. If an instance with the same TrustyClaw tag exists, deploy asks
for confirmation before terminating it.
...
2. **Over SSH with the deploy key**: deploy copies the runtime code and a bootstrap
   ...
   proxy CA, installs the systemd services, applies the initial network policy,
   and finally deletes the deploy key.

SSH port 22 must be open because SSH tunneling is currently the only supported
way to access the admin UI and API. Deploy rejects configs that set
...
| `/mnt/trustyclaw-admin/proxy-state/network_status.json` | proxy only | Policy status: `loading`, `reloading`, or `active`. |
...
`/v1/health` does not trust the recorded policy status alone: it also confirms the
proxy process is actually listening. If the proxy has died while the policy files
still say `active`, health reports `network_controls.status: error` so the operator
notices and restarts it. This failure is safe — nftables blocks the agent's direct
egress independently of the proxy, so a dead proxy leaves the agent with no network
at all, never with unfiltered access.
...
root helper, which demotes to `trustyclaw-proxy`, writes status `reloading`,
swaps the policy, and writes `active`. The proxy denies all requests whenever
status is not `active`. The same helper applies the initial policy at the end of
bootstrap, so the proxy starts fail-closed and opens only once the configured
policy is in place.
