...
lives in the local Postgres database whose data directory sits on this volume
under `postgres/`, owned by the `postgres` user (see
...
narrow database role over exactly them); only the proxy's TLS material — the
CA keypair and minted leaf certificates — remains under the proxy-owned
`proxy-state/` directory, mode 700, because the `ssl` module and `openssl`
consume file paths and the CA private key stays out of the admin-owned
schema. Bootstrap sanitizes managed paths before writing them on redeploy —
