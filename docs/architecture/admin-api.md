...
filesystem authority and leaves no partial file at the returned task path.

The orchestrator runs nine worker threads, with a claim cap of three tasks per
agent runtime.
...
- Every task names a client-chosen `thread_id`. The first task also names an
  `agent_runtime` (`codex`, `claude_code`, or `hermes`) and one allowed
  ...
  pairs run in parallel, up to nine total and up to three per runtime.
- Each running task gets its own runtime process, spawned through the sudo
  ...
  thread by id on a fresh app-server; Claude Code and Hermes turns resume
  by session id.
