...
| `oauth_logins` | In-flight OAuth logins, fully typed per flow (device code + login handle for Codex, browser code for Claude). The Bedrock harnesses have no row here: their credential connect is a single API call with nothing in flight. |
...
| `bedrock_credentials` | One singleton connection shared by Pi and Hermes: the synchronously validated IAM `access_key_id`, secret access key as `secretbox` ciphertext, and selected region. The admin service writes all three atomically; the trusted proxy reads this same row only for enabled Bedrock requests and re-signs requests carrying a harness's fixed dummy access-key id. |
...
| `proxy_provider_pins` | Exactly the two values the proxy guards compare: account id and token hash (format-checked). Rows exist for the OAuth runtimes (`codex`, `claude`); the Bedrock harnesses have no pin because the proxy reads their single validated credential row directly. |
