...
| `aws_session_token_env` | No | string | Name of the environment variable containing an AWS session token. Set it when the credentials are temporary (an assumed STS role); omit it for long-lived access keys. Every lifecycle command accepts it. |
...
All lifecycle commands accept the optional `aws_session_token_env` field, so
temporary role credentials work uniformly across deploy, upgrade, recover,
reconfigure, start, and stop.
