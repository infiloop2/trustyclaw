  ...
  "tools": [
    {
      ...
      "setup_guide": "Create an OAuth client for a dedicated account, register <admin origin>/oauth/callback as a redirect URI, then set the config values here, enable, and connect.",
...
| `actions[]` | Each action's stable `id`, `description`, per-action `data_policy`, `input_schema`, and `output_schema` (empty `{}` for approval-gated actions, which return a user-visible message rather than a JSON result). |
...
| `setup_guide` | A single operator-facing string: what to do on the provider side before configuring and connecting; surfaced in the tool's info popover. |
